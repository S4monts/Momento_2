...
public class viewExampleController implements Initializable {
   ...
   @FXML
    ...
    @FXML
    private void Actionx(ActionEvent event){
    ...
    @FXML
    private void Actiony(ActionEvent event){
        ...
        @FXML
    private void Actionz(ActionEvent event){
    try {
        ...
        @FXML
    private void Actiona(ActionEvent event){
    try {
        LocalDate FechaVencimiento = LocalDate.parse(JOptionPane.showInputDialog("Ingrese la Fecha Lote que desea buscar (dd/MM/yyyy)"));
...
JOptionPane.showMessageDialog(null, "Formato de fecha inválido, por favor ingrese una fecha en el formato dd/MM/yyyy", "Fecha Inválida", JOptionPane.ERROR_MESSAGE);
    ...
    if (MayorPrecio != null) {
      ...
      @FXML
    private void menorpecio(ActionEvent event){
...
// PROMEDIO
    ...
    @FXML
    private void menorpromedio(ActionEvent event){
    ...
    //tabla-acción
    @FXML
    private TextField IDLITextField,NomLTextField,precioTextField;
    ...
    private void eventKey(KeyEvent event){
        ...
        if(evt.equals(IDLITextField)){
        
            if(!Character.isDigit(event.getCharacter().charAt(0))){
                event.consume();
            }
           
         }else if (evt.equals(NomLTextField)){
            
            if(!Character.isLetter(event.getCharacter().charAt(0))){
                event.consume();
            }
            
        }else if (evt.equals(precioTextField)){ 
    ...
    String IdP = IDLITextField.getText();
    String NomP = NomLTextField.getText();
    double Precio = Double.parseDouble(precioTextField.getText());
    LocalDate FechLo = FechaLote.getValue();
    LocalDate FechVen = FechaVe.getValue();
